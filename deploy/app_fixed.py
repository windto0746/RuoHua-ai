...
def build_ui():
    ...
    with gr.Blocks(title="短视频AI文案生成器 v9.0", css=custom_css) as app:
        gr.HTML("""
        <div class="header-section">
            <h1>短视频AI文案生成器</h1>
            <p style="color:rgba(255,255,255,0.9);margin-top:8px;">v9.0 | 通义千问AI | 专业Prompt驱动</p>
        </div>
        """)
        ...
        with gr.Row():
            with gr.Column(scale=2):
                ...
                gr.HTML('<div style="background:linear-gradient(135deg,#F0FFF4,#E6FFE6);border-radius:14px;padding:20px;margin-top:20px;border:2px solid #90EE90;"><div class="section-title">✏️ 生成结果</div></div>')
            ...
            with gr.Column(scale=1):
                gr.HTML('<div style="background:white;border-radius:16px;padding:22px;box-shadow:0 4px 20px rgba(74,144,226,0.15);margin-bottom:18px;"><div class="section-title">🔥 热门话题</div>')
                for category, topics in HOT_TOPICS.items():
                    gr.HTML(f'<div style="margin:12px 0 8px 0;font-weight:600;color:#4A90E2;">{category}</div>')
                    gr.HTML(f'<div>{" ".join([f"<span style='display:inline-block;background:linear-gradient(135deg,#4A90E2,#67B8F5);color:white;padding:8px 16px;border-radius:20px;margin:4px;font-size:13px;'>{t}</span>" for t in topics])}</div>')
                gr.HTML('</div>')
